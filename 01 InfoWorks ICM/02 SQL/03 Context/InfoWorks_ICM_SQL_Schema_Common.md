...
| Node depth/invert | `chamber_floor_level` | `maximum_depth` | InfoWorks uses `chamber_floor_level`; SWMM uses `maximum_depth` |
...
Field not indexed here → see `InfoWorks_ICM_Database_Fields_Guide.md` Step 2 (MCP lookup). For the local workflow, search Autodesk Help using `{Object Type} Data Fields` and use the **Database field** column value; do not use the UI **Field Name**.
