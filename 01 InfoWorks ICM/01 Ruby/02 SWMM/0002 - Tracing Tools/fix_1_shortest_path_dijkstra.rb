...
# Purpose: Dijkstra shortest path algorithm for ICM networks
# Inputs: UI script; requires 2 selected nodes (start and target)
# Outputs: Selects the shortest path of nodes and links between them
# Type: UI script (runs in ICM context)
# Hardening: Begin/rescue/ensure for error handling, input validation, visited set for cycle detection
...
begin
  ...
  puts "Shortest path trace completed. #{nodes.size} nodes processed."

rescue => e
  puts "Error in shortest path trace: #{e.message}"
