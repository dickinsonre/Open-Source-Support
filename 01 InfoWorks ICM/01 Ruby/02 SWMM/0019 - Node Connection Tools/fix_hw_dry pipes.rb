...
# fix_hw_dry pipes.rb
# -----------------------------------------------------------------------------
# Purpose : Select 'dry pipes' in an InfoWorks (hw_*) network: links that are
#           never reached by traversing downstream from any subcatchment
#           outlet node.
# Inputs  : Current open hw_* network with subcatchments and links.
...
#   - Resets the `_seen` flag on every link/node up front so re-running the
#     script in the same session is reliable
...
begin
  puts "[#{ts}] Starting hw_ dry-pipes detection."
  ...
  # Seed traversal queue from each subcatchment outlet node
  unprocessed_links = []
  subs.each do |sub|
    nid = sub.node_id
    next if nid.nil? || nid.to_s.empty?
    begin
      nd = net.row_object('hw_node', nid)
  ...
  # Anything still unseen is a dry pipe
  dry_links = 0
  all_links.each do |d|
    next if d._seen
    d.selected = true
