...
# Purpose: Import scenarios from InfoSWMM CSV files
# Inputs: UI script; user folder selection for Scenario.csv
# Outputs: Creates scenarios in current network; deletes existing non-Base scenarios
# Type: UI script (runs in ICM context)
# Hardening: Begin/rescue/ensure, file existence check, user input validation, transaction control
...
def import_scenario(open_net)
  ...
  raise "Scenario.csv not found in the provided folder" unless File.exist?(scenario_csv)
  ...
  if val&.first.nil? || val&.first&.strip&.empty?
...
begin
  ...
  raise 'No scenarios imported' if sorted_rows.nil? || sorted_rows.empty?
  ...
  puts "Error importing scenarios: #{e.message}"
