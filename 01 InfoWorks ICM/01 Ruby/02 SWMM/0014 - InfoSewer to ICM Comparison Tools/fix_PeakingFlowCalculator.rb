...
# ============================================================================
# FIXED: Design Flow Calculator - PEAKABLE/UNPEAKABLE SEPARATION
# ============================================================================
# Purpose: Separate peakable (trade) and unpeakable (base) flows; calculate
#   peaking factors using configurable EFF formulas (Harmon, Modified Harmon,
#   Babbitt); export results to CSV with flow components and topology.
# Inputs: Network with subcatchments (population), conduits (width/height).
# Outputs: Conduit fields (trade_flow, base_flow, user_number_1); CSV export.
# UI/EX: Dialog prompts formula preset, per-capita flows, EFF cutoff; shows
#   parameter validation, topology stats, per-conduit results; exports
#   design_flows_TIMESTAMP.csv.
# Hardening:
#   - frozen_string_literal; begin/rescue/ensure wrapper
#   - Network nil-check, transaction safety
#   - Validation of all user inputs (range checks, zero-division guards)
#   - Regex safeguard on section headers (line.start_with?, line.end_with?)
#   - File.open block form for CSV (when applicable)
#   - Progress logging every 100 conduits
#   - Skip malformed lines with warnings (not crash)
#   - Nil-safety on population traces, flow arrays
# ============================================================================
...
layout = [
  ...
  ['Formula Preset for Peaking', 'String', 'Harmon Formula', nil, 'LIST', preset_options],
  ['Peakable flow per capita (L/person/day)', 'NUMBER', 150.0, 2],
  ['Maximum EFF cutoff', 'NUMBER', 6.0, 2],
  ['Base flow per capita (L/person/day)', 'NUMBER', 50.0, 2],
  ...
  ['Show detailed topology stats', 'Boolean', true],
...
begin
  ...
  puts "[X] Error accessing network: #{e.message}"
  ...
  begin
    ...
    if us_node
      ...
      us_node.us_links.each do |ul|
        us_conduit_count += 1 if ul && ul.table == 'hw_conduit'
      end
  ...
  begin
    ...
    if ds_node
      ...
      ds_node.ds_links.each do |dl|
        ds_conduit_count += 1 if dl && dl.table == 'hw_conduit'
      end
  ...
  while unprocessed_links.size > 0 && iteration_count < max_iterations
    ...
    begin
      us_node.ds_links.each do |dl|
        downstream_conduits << dl if dl && dl.table == 'hw_conduit'
      end
    rescue
    end
    ...
    if node_sub_hash_map[node_id]
      node_sub_hash_map[node_id].each do |sub|
        if sub && sub.population && sub.population > 0
    ...
    begin
      ...
      unless dry_run
        begin
          ...
          errors += 1
          puts "  [X] Error writing conduit #{conduit_id}: #{e.message}" if debug_mode
          next
        end
      end
...
if export_csv && results.size > 0
  ...
  csv_paths_to_try = []
  csv_paths_to_try << File.join(documents_path, filename) if documents_path && File.directory?(documents_path)

  desktop_path = File.join(ENV['USERPROFILE'] || ENV['HOME'] || '', 'Desktop')
  csv_paths_to_try << File.join(desktop_path, filename) if desktop_path && File.directory?(desktop_path)
