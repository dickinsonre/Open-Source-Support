...
# ----------------------------------------------------------------------------
# Purpose : Hardened wrapper around "Compare ICM trade flow to SWMM Base
#           Flow.rb". Original compares HW subcatchment trade_flow / base_flow
#           / additional_foul_flow / population against SW node base_flow,
#           with options to copy values across and to materialize ICM
#           subcatchments from SWMM nodes.
# Inputs  : current_network (HW), background_network (SW), prompt selections.
# Outputs : Console comparison report; optional in-place updates wrapped in
#           a transaction (preserved in original).
# Type    : UI script - runs inside InfoWorks ICM.
# Hardening notes:
#   * frozen_string_literal pragma.
#   * begin/rescue/ensure outer guard, including transaction_rollback if a
#     transaction was active when failure occurred.
#   * Validates current_network AND background_network are loaded.
#   * Catches Interrupt for user cancel; SystemExit propagates exit codes.
#   * Behavior of underlying comparison logic is preserved verbatim.
# ============================================================================

require 'time'

ORIGINAL_SCRIPT = File.join(__dir__, 'Compare ICM trade flow to SWMM Base Flow.rb').freeze
