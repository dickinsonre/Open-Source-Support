...
begin
  ...
  puts "[#{Time.now.strftime('%H:%M:%S')}] Results fields summary"
  count = 0
  net.tables&.each do |t|
    net.row_objects(t.name)&.each do |ro|
      next if ro.nil? || ro.table_info.nil? || ro.table_info.results_fields.nil?
      count += ro.table_info.results_fields.length
    end
  end
