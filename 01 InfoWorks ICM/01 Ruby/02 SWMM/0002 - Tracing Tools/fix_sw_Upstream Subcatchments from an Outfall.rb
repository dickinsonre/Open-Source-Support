...
# Purpose: Select upstream subcatchments from SWMM node with hash acceleration
...
begin
  ...
  # Create a hash variable for outlet_id->subcatchment mapping
  node_sub_hash_map = Hash.new { |h, k| h[k] = [] }

  # Get all nodes from the network
  all_nodes = net.row_objects('_nodes')

  # Pair subcatchments to outlets (SWMM-specific)
  all_subs.each do |subb|
    if subb.outlet_id && !subb.outlet_id.empty?
